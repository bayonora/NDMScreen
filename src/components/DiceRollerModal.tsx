      ...
      <div className="flex flex-col md:flex-row gap-8 h-[450px] w-full">
        ...
        <div className="flex-1 flex flex-col gap-8">
          ...
          <div className="flex flex-col gap-3">
            <label className="text-xs font-bold text-[#c1a063] uppercase tracking-widest text-center">
              Selecciona el Dado
            </label>
              ...
              {DICE_TYPES.map(d => (
                <button
                  ...
                  className={cn(
                    ...
                    selectedDie === d 
                      ? "bg-[#c1a063] text-[#0f0d0c] border-[#c1a063] shadow-[0_0_15px_rgba(193,160,99,0.4)] scale-105" 
                      : "bg-[#14110f] border-[#3a302a] text-[#8b7355] hover:border-[#c1a063] hover:text-[#c1a063]"
                  )}
                >
                  <span>d{d}</span>
                </button>
              ...
              <div
                className={cn(
                  ...
                  selectedDie === customDie && String(customDie) !== ""
                    ? "bg-[#c1a063] text-[#0f0d0c] border-[#c1a063] shadow-[0_0_15px_rgba(193,160,99,0.4)] scale-105" 
                    : "bg-[#1e1a17] border-dashed border-[#5a4a40] text-[#8b7355] focus-within:border-solid focus-within:border-[#c1a063] focus-within:text-[#c1a063] shadow-inner"
                )}
                onClick={() => {
          ...
          <div className="flex flex-col gap-3 items-center">
            <label className="text-xs font-bold text-[#c1a063] uppercase tracking-widest">
              Cantidad
            </label>
            <div className="flex items-center gap-6 bg-[#14110f] border border-[#3a302a] rounded-full p-2 px-4 shadow-inner w-full max-w-[200px] justify-between">
              <button 
                type="button"
                onClick={() => setQuantity(Math.max(1, quantity - 1))}
                className="w-10 h-10 flex items-center justify-center text-[#8b7355] hover:text-[#c1a063] hover:bg-[#1e1a17] rounded-full transition-colors font-bold text-xl"
              >
                -
              </button>
              <span className="text-3xl font-display font-bold text-[#e6e2da] w-12 text-center">
                {quantity}
              </span>
              <button 
                type="button"
          ...
          <div className="flex justify-center mt-auto pt-4">
            <button 
              ...
              className={cn(
                ...
                isRolling 
                  ? "bg-[#14110f] text-[#8b7355] border-[#3a302a] cursor-not-allowed" 
                  : "bg-[#c1a063] text-[#0f0d0c] border-[#c1a063] hover:bg-[#d4b57a] shadow-[0_0_15px_rgba(193,160,99,0.3)] hover:shadow-[0_0_20px_rgba(193,160,99,0.5)]"
              )}
            >
              {isRolling ? (
                <motion.div
        ...
        <div className="flex-1 flex flex-col bg-[#14110f] border border-[#3a302a] rounded-sm p-4 relative h-full">
          <div className="flex items-center justify-between mb-4 border-b border-[#3a302a] pb-2">
            <h3 className="text-[#c1a063] font-bold uppercase tracking-widest text-sm flex items-center gap-2">
            ...
            <AnimatePresence>
              {history.length === 0 && !isRolling && (
                <motion.div 
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="text-center text-[#8b7355] italic m-auto opacity-50"
                >
              ...
              {history.map((record, index) => (
                <motion.div
                  ...
                  className={cn(
                    ...
                    index === 0 
                      ? "bg-[#1e1a17] border-[#c1a063] shadow-md" // Highlight latest
                      : "bg-[#1a1614] border-[#3a302a] opacity-80"
                  )}
                >
                    ...
                    {record.results.map((res, i) => (
                      <div 
                        ...
                        className={cn(
                          ...
                          res === record.die 
                            ? "bg-[#c1a063] text-[#0f0d0c] border-[#c1a063]" // Crit Success
                            : res === 1
                            ? "bg-[#8a211b] text-[#e6e2da] border-[#8a211b]" // Crit Fail
                            : "bg-[#14110f] text-[#e6e2da] border-[#3a302a]" // Normal
                        )}
                      >
                        {res}
                      </div>
                  ...
                  {record.quantity > 1 && (
                    <div className="mt-1 pt-2 border-t border-[#3a302a]/50 text-right">
                      <span className="text-[#8b7355] text-xs uppercase tracking-widest mr-2">Total</span>
                      <span className="text-xl font-bold text-[#c1a063]">{record.total}</span>
                    </div>
                  )}
                </motion.div>
              ))}
